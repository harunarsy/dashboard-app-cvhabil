...
const ensureSchema = async () => {
  ...
  `);
  // v1.64.0 — Buku Besar 2.0 (metode Excel Harun: mutasi bank → kategori → alokasi amplop bulanan).
  await pool.query(`ALTER TABLE ledger_entries ADD COLUMN IF NOT EXISTS tax_scope TEXT DEFAULT 'ppn'`).catch(() => {});      // 'ppn' | 'non_ppn' (dus lipat)
  await pool.query(`ALTER TABLE ledger_entries ADD COLUMN IF NOT EXISTS source TEXT DEFAULT 'manual'`).catch(() => {});      // 'manual' | 'estatement'
  await pool.query(`ALTER TABLE ledger_entries ADD COLUMN IF NOT EXISTS bank_ref TEXT`).catch(() => {});                     // kunci dedup import
  await pool.query(`ALTER TABLE ledger_entries ADD COLUMN IF NOT EXISTS needs_review BOOLEAN DEFAULT FALSE`).catch(() => {});
  await pool.query(`ALTER TABLE ledger_entries ADD COLUMN IF NOT EXISTS auto_cat BOOLEAN DEFAULT FALSE`).catch(() => {});
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_ledger_bankref ON ledger_entries(bank_ref) WHERE bank_ref IS NOT NULL`).catch(() => {});
  // Amplop alokasi: pct × laba kotor bulan; match_categories = kategori ledger yg dihitung sbg pengeluaran amplop ini.
  await pool.query(`
  ...
  `).catch(() => {});
  // tabel employees versi lama (email/department/salary) mungkin sudah ada → tambal kolom baru.
  await pool.query(`ALTER TABLE employees ADD COLUMN IF NOT EXISTS role TEXT`).catch(() => {});
  await pool.query(`ALTER TABLE employees ADD COLUMN IF NOT EXISTS daily_wage NUMERIC(15,2) DEFAULT 0`).catch(() => {});
  await pool.query(`ALTER TABLE employees ADD COLUMN IF NOT EXISTS monthly_salary NUMERIC(15,2) DEFAULT 0`).catch(() => {});
  await pool.query(`ALTER TABLE employees ADD COLUMN IF NOT EXISTS active BOOLEAN DEFAULT TRUE`).catch(() => {});
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_employees_name ON employees(name)`).catch(() => {});
  await pool.query(`
  ...
  `).catch(() => {});
  // Hutang piutang DI LUAR transaksi dagang (mis. pinjaman pribadi Direktur). amount: + = pinjam
  // (nambah hutang), − = cicil/kembali. Running total dihitung per orang.
  await pool.query(`
