    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999 }}>
      <div onClick={e => e.stopPropagation()} className="glass-target glass-target--clear ui-motion-modal" style={{ backgroundColor: bg, borderRadius: '16px', width: '100%', maxWidth: '360px', overflow: 'hidden', boxShadow: '0 32px 64px rgba(0,0,0,0.35)', padding: '24px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={onClose} className="ui-motion-button ui-focus-ring" style={{ flex: 1, padding: '12px', backgroundColor: isDarkMode ? '#2C2C2E' : '#F5F5F7', color: text, border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Batal</button>
          <button onClick={() => { onConfirm(); onClose(); }} className="ui-motion-button ui-focus-ring" style={{ flex: 1, padding: '12px', backgroundColor: '#FF3B30', color: '#FFF', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '700' }}>Hapus</button>
        </div>
      </div>
    </div>
  );
