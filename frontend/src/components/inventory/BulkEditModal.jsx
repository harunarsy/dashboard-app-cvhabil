...
export default function BulkEditModal({ products, allProducts, allCategories, onClose, onSaved, isDarkMode }) {
  ...
  const duplicates = useMemo(() => {
    ...
    return rows
      .filter(r => r.codeNew.trim() && allCodes.get(r.codeNew.trim()) === 'DUPE')
      .map(r => r.id);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        await inventoryAPI.updateProduct(r.id, {
          code: mode !== 'category' ? r.codeNew.trim() || null : (full.code || null),
          name: full.name,
          unit: full.unit,
          ...
          category: mode !== 'code' ? r.categoryNew.trim() : (full.category || ''),
          min_stock: full.min_stock || 5,
        });
        success.push(r.id);
      } catch (e) {
        failed.push({ id: r.id, name: r.name, error: e.response?.data?.error || e.message });
      }
      setProgress({ done: i + 1, total: rows.length });
    }
    setSaving(false);
  ...
  const cellStyle = { padding: '8px 10px', fontSize: '12px', verticalAlign: 'middle' };
  const inputStyle = {
    width: '100%', padding: '6px 10px', border: `1px solid ${border}`,
    borderRadius: '8px', background: bg, color: text, fontSize: '12px',
  ...
  return (
    ...
    >
      <div className="glass-target glass-target--clear" style={{
        ...
        {/* Mode selector */}
        <div style={{ padding: '14px 22px 8px', borderBottom: `1px solid ${border}` }}>
          <div style={{ display: 'inline-flex', background: surface, borderRadius: '8px', padding: '3px' }}>
            {[
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
            <thead>
              <tr style={{ background: cellBg }}>
                <th style={{ ...cellStyle, textAlign: 'left', fontWeight: '700', color: sub, borderBottom: `1px solid ${border}` }}>Nama Produk</th>
                {mode !== 'category' && (
                  <th style={{ ...cellStyle, textAlign: 'left', fontWeight: '700', color: sub, borderBottom: `1px solid ${border}`, width: '34%' }}>Kode</th>
                )}
                {mode !== 'code' && (
                  <th style={{ ...cellStyle, textAlign: 'left', fontWeight: '700', color: sub, borderBottom: `1px solid ${border}`, width: '34%' }}>Kategori</th>
                )}
              </tr>
            </thead>
            <tbody>
                    ...
                    {mode !== 'code' && (
                      <td style={cellStyle}>
                        <input
                          type="text"
                          value={r.categoryNew}
                          onChange={e => updateRow(r.id, 'categoryNew', e.target.value)}
                          disabled={saving}
                        ...
                        {r.categoryOld && r.categoryOld !== r.categoryNew && (
                          <p style={{ margin: '2px 0 0', fontSize: '10px', color: sub }}>lama: {r.categoryOld}</p>
                        )}
                      </td>
                    )}
                  </tr>
        ...
        {/* Footer */}
        <div style={{ padding: '14px 22px', borderTop: `1px solid ${border}`, display: 'flex', gap: '10px', alignItems: 'center', justifyContent: 'space-between' }}>
          {saving ? (
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={onClose} disabled={saving} style={{
              padding: '10px 18px', background: surface, color: text, border: `1px solid ${border}`,
              borderRadius: '10px', fontWeight: '600', fontSize: '13px', cursor: saving ? 'wait' : 'pointer',
            }}>Tutup</button>
            {!results && (
