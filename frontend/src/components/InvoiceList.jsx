...
export default function InvoiceList({ isDarkMode, isSidebarOpen }) {
  ...
  useEffect(() => { applyFilters(); }, [invoices, universalSearch, selectedMonth, searchDist, searchInv, filterStatus, filterDue, dateFrom, dateTo]);
  ...
  // ── Autosave draft every 30s when modal is open ──
  const autosaveDraft = useCallback(async (f, it) => {
    try {
      await invoicesAPI.saveDraft({ form: f, items: it });
    } catch (e) {}
  }, []);
  ...
  useEffect(() => {
    ...
    draftTimerRef.current = setInterval(() => {
      autosaveDraft(form, items);
    }, 30000);
    return () => clearInterval(draftTimerRef.current);
  }, [showModal, form, items, autosaveDraft]);
  ...
  const applyFilters = () => {
    ...
    if (selectedMonth !== 'all') {
      f = f.filter(i => new Date(i.purchase_date).toLocaleString('id-ID', { month: 'long', year: 'numeric' }) === selectedMonth);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) await invoicesAPI.update(editingId, payload);
  ...
  const resetForm = () => {
    setForm(blankForm()); setItems([blankItem()]); setEditingId(null);
  };
  ...
  return (
    <div style={{ padding: '2rem', marginLeft: isSidebarOpen ? '256px' : '80px', backgroundColor: isDarkMode ? '#000000' : '#F5F5F7', minHeight: '100vh', transition: 'margin-left 0.3s' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <button onClick={openNewInvoice} style={{ padding: '10px 20px', backgroundColor: '#007AFF', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      {/* ── Universal Search + Filter Toggle ── */}
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px', paddingTop: '4px' }}>
              ...
              <div>
                ...
                <input style={S.input} value={searchDist} onChange={e => setSearchDist(e.target.value)} placeholder="Cari..." />
              </div>
              <div>
                <label style={S.label}>No Faktur</label>
                <input style={S.input} value={searchInv} onChange={e => setSearchInv(e.target.value)} placeholder="Cari..." />
              </div>
              <div>
      ...
      <div style={{ ...S.card, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '150px 1fr 110px 130px 130px 150px 110px 120px', padding: '12px 16px', backgroundColor: isDarkMode ? '#2C2C2E' : '#F5F5F7', borderBottom: `1px solid ${isDarkMode ? '#3A3A3C' : '#E5E5EA'}` }}>
          {['No Faktur', 'Distributor', 'Tgl Faktur', 'HNA*QTY', 'HNA Final', 'HNA+PPN / HPP', 'Status', 'Aksi'].map(h => (
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ backgroundColor: isDarkMode ? '#1C1C1E' : '#FFF', borderRadius: '16px', padding: '28px', maxWidth: '360px', width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', textAlign: 'center' }}>
            ...
            <p style={{ margin: '0 0 6px', fontSize: '14px', color: '#86868B' }}>
              Faktur <strong style={{ color: isDarkMode ? '#FFF' : '#000' }}>{deleteConfirm.name}</strong> akan dipindahkan ke trash.
            </p>
...
function InvoiceRow({ inv, isDarkMode, expanded, onToggleExpand, onEdit, onDelete, formatRp }) {
  ...
  return (
    <>
      <div
        style={{ display: 'grid', gridTemplateColumns: '150px 1fr 110px 130px 130px 150px 110px 120px', padding: '14px 16px', borderBottom: `1px solid ${isDarkMode ? '#2C2C2E' : '#F0F0F0'}`, alignItems: 'center', backgroundColor: hovered ? (isDarkMode ? '#2C2C2E' : '#F9F9F9') : (isDarkMode ? '#1C1C1E' : '#FFF'), transition: 'background 0.15s' }}
      ...
      >
        ...
        <div>
          ...
          {dueStatus && (
            <div style={{ marginTop: '4px', display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '2px 8px', borderRadius: '8px', backgroundColor: dueStatus.bg }}>
        ...
        <div>
          ...
          {inv.payment_date && <div style={{ fontSize: '11px', color: '#86868B', marginTop: '2px' }}>Bayar: {new Date(inv.payment_date).toLocaleDateString('id-ID')}</div>}
          {inv.due_date && inv.status !== 'Paid' && <div style={{ fontSize: '11px', color: '#86868B', marginTop: '2px' }}>Jatuh tempo: {new Date(inv.due_date).toLocaleDateString('id-ID')}</div>}
        </div>
        ...
        <div>
          ...
          {inv.harga_per_produk > 0 && (
            <div style={{ fontSize: '11px', color: '#AF52DE', fontWeight: '600', marginTop: '2px' }}>
              HPP/item: {formatRp(inv.harga_per_produk)}
            </div>
          )}
        </div>
  ...
  return (
    <div style={{ backgroundColor: isDarkMode ? '#111' : '#FAFAFA', borderBottom: `1px solid ${isDarkMode ? '#2C2C2E' : '#E5E5EA'}`, padding: '8px 24px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: cols, gap: '8px', padding: '6px 0', borderBottom: `1px solid ${isDarkMode ? '#2C2C2E' : '#E5E5EA'}`, marginBottom: '4px' }}>
        {['Nama Produk', 'QTY', 'HNA', 'HNA*QTY', 'Disc%', 'Disc Nominal', 'HNA Baru', 'HNA/Item'].map(h => (
      ...
      <div onClick={e => e.stopPropagation()} style={{ backgroundColor: isDarkMode ? '#1C1C1E' : '#FFF', borderRadius: '16px', width: '100%', maxWidth: '780px', boxShadow: '0 32px 64px rgba(0,0,0,0.35)', overflow: 'hidden', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: `1px solid ${isDarkMode ? '#2C2C2E' : '#E5E5EA'}`, backgroundColor: isDarkMode ? '#000' : '#F5F5F7' }}>
          <div>
            ...
            <p style={{ margin: '2px 0 0', fontSize: '13px', color: '#86868B' }}>Draft tersimpan otomatis setiap 30 detik</p>
          </div>
        ...
        <div style={{ padding: '24px' }}>
          ...
          <div style={sec}>
            ...
            <div style={r2}>
              <div>
                <label style={S.label}>No Faktur</label>
                <input style={S.input} value={form.invoice_number} onChange={e => onFormChange('invoice_number', e.target.value)} placeholder="Contoh: 1260300020" />
              </div>
              <div>
                <label style={S.label}>Tanggal Belanja / Faktur</label>
                <input type="date" style={S.input} value={form.purchase_date} onChange={e => onFormChange('purchase_date', e.target.value)} />
              </div>
            </div>
            <div style={{ ...r2, marginTop: '14px' }}>
              <div>
                <label style={S.label}>Distributor</label>
                <MasterSelect value={form.distributor_name} onChange={v => onFormChange('distributor_name', v)} options={distributors} onAdd={onAddDistributor} onRemove={onRemoveDistributor} placeholder="Pilih atau tambah distributor..." isDarkMode={isDarkMode} />
              </div>
              <div>
          ...
          <div style={sec}>
            ...
            {items.map((item, idx) => (
              <div key={item._id} style={{ backgroundColor: isDarkMode ? '#2C2C2E' : '#F9F9FB', borderRadius: '12px', padding: '14px', marginBottom: '10px', border: `1px solid ${isDarkMode ? '#3A3A3C' : '#E5E5EA'}`, position: 'relative' }}>
                {items.length > 1 && (
                  <button type="button" onClick={() => removeItem(idx)} style={{ position: 'absolute', top: '10px', right: '10px', padding: '5px', backgroundColor: '#FF3B30', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
                    <X size={12} />
                  </button>
                )}
                <div style={{ ...r2, marginBottom: '10px' }}>
                  <div>
                    ...
                    <MasterSelect value={item.product_name} onChange={v => updateItem(idx, 'product_name', v)} options={products} onAdd={onAddProduct} onRemove={onRemoveProduct} placeholder="Pilih atau tambah produk..." isDarkMode={isDarkMode} />
                  </div>
                  <div>
                    <label style={S.label}>Expired Date</label>
                    <input type="date" style={S.input} value={item.expired_date} onChange={e => updateItem(idx, 'expired_date', e.target.value)} />
                  </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                  ...
                  <div>
                    <label style={S.label}>HNA</label>
                    <input style={S.input} value={item.hna === '' ? '' : formatRpInput(parseNum(item.hna))} onChange={e => { const raw = e.target.value.replace(/\./g, '').replace(/[^0-9]/g, ''); updateItem(idx, 'hna', raw); }} placeholder="Rp 0" />
                  </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '10px' }}>
                  <div><label style={{ ...S.label, color: '#86868B' }}>HNA × QTY</label><input style={computed} value={formatRpInput(item.hna_times_qty)} readOnly /></div>
                  ...
                  <div><label style={{ ...S.label, color: isDarkMode ? '#30D158' : '#1C7C2A' }}>HNA Baru</label><input style={computed} value={formatRpInput(item.hna_baru)} readOnly /></div>
                  <div><label style={{ ...S.label, color: '#AF52DE' }}>HNA/Item</label><input style={{ ...computed, color: '#AF52DE' }} value={formatRpInput(item.hna_per_item)} readOnly /></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={sec}>
            ...
            <div style={r2}>
              <div><label style={S.label}>HNA×QTY Total</label><input style={computed} value={formatRpInput(totals.total_hna)} readOnly /></div>
            ...
            <div style={{ ...r2, marginTop: '12px' }}>
              <div><label style={S.label}>HNA Baru (HNA−DISC)</label><input style={computed} value={formatRpInput(totals.hna_baru)} readOnly /></div>
              <div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <input style={form.disc_cod_ada ? S.input : S.inputDis} disabled={!form.disc_cod_ada}
                    ...
                    onChange={e => { const raw = e.target.value.replace(/\./g, '').replace(/[^0-9]/g, ''); onFormChange('disc_cod_amount', raw); }}
                    placeholder="Rp 0" />
                </div>
              </div>
            </div>
            <div style={{ ...r2, marginTop: '12px' }}>
              <div><label style={S.label}>HNA Final (HNA Baru − Disc COD)</label><input style={computed} value={formatRpInput(totals.hna_final)} readOnly /></div>
            ...
            <div style={{ ...r2, marginTop: '12px' }}>
              ...
              <div><label style={{ ...S.label, color: '#007AFF' }}>HNA + PPN Masukan</label><input style={{ ...computed, color: '#007AFF', fontSize: '15px', fontWeight: '700' }} value={formatRpInput(totals.hna_plus_ppn)} readOnly /></div>
            </div>
