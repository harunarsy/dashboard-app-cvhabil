...
export default function SalesOrderList({ isDarkMode, isSidebarOpen }) {
  ...
  // Form state
  const [form, setForm] = useState({ customer_name: '', customer_address: '', sale_date: new Date().toISOString().split('T')[0], notes: '' });
  ...
  const openAdd = () => {
    setEditId(null);
    setForm({ customer_name: '', customer_address: '', sale_date: new Date().toISOString().split('T')[0], notes: '' });
    setItems([blankItem()]);
    setShowModal(true);
  };
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          <thead>
            <tr style={{ backgroundColor: isDarkMode ? '#1C1C1E' : '#F5F5F7' }}>
              {['No. Nota', 'Tanggal', 'Customer', 'Total', 'Status', 'Aksi'].map(h => (
                ...
                <tr style={{ borderBottom: `1px solid ${border}`, cursor: 'pointer' }} onClick={() => setExpandedId(expandedId === o.id ? null : o.id)}>
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button onClick={(e) => { e.stopPropagation(); openEdit(o); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}><Edit2 size={15} color="#007AFF" /></button>
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(o.id); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}><Trash2 size={15} color="#FF3B30" /></button>
                    </div>
                  </td>
                </tr>
        ...
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '1rem' }}>
          <div onClick={e => e.stopPropagation()} style={{ backgroundColor: cardBg, borderRadius: '16px', width: '100%', maxWidth: '640px', maxHeight: '90vh', overflow: 'auto', boxShadow: '0 32px 64px rgba(0,0,0,0.35)' }}>
            ...
            <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {/* Date */}
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: sub, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>Tanggal</label>
